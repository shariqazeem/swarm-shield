...
function encryptIntent(
  ...
  // Generate ephemeral keypair for this intent
  ...
  encrypted.set(ciphertext, 56);                  // 40 bytes
...
// Output: 9bed43a48f60f39e2a857226... (96 bytes)
```
...
class SwarmShieldAgent {
  ...
  async submitEncryptedIntent(
    ...
    // (See full implementation in API Reference)
...
async function main() {
  ...
  const encrypted = encryptIntent(
    ...
    BigInt(9500000)        // Min 9.5 USDC
  );

  console.log("Encrypted intent ready:", encrypted.length, "bytes");

  // Submit to program (simplified)
  // See API Reference for full transaction building
}
